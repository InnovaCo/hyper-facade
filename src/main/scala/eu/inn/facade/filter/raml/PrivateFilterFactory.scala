...
class PrivateFilterFactory(config: Config) extends RamlFilterFactory {
  ...
  def createFilterChain(target: RamlTarget): SimpleFilterChain = {
    target match {
      ...
      case TargetResource(_, Annotation(_, Some(privateArgs: private_))) ⇒ SimpleFilterChain(
        requestFilters = Seq(new RequestPrivateFilter(privateAddresses, privateArgs)),
        responseFilters = Seq.empty,
        eventFilters = Seq.empty
      )

      case TargetMethod(_, _, Annotation(_, Some(privateArgs: private_))) ⇒ SimpleFilterChain(
        requestFilters = Seq(new RequestPrivateFilter(privateAddresses, privateArgs)),
        responseFilters = Seq.empty,
        eventFilters = Seq.empty
      )
